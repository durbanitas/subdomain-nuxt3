...
import { $ as $fetch, l as hasProtocol, m as isScriptProtocol, n as joinURL, w as withQuery, o as defu, p as sanitizeStatusCode, q as createHooks, h as createError$1, t as toRouteMatcher, r as createRouter$1, v as klona, x as parse, y as getRequestHeader, z as destr, A as isEqual, B as setCookie, C as getCookie, D as deleteCookie } from '../runtime.mjs';
...
const _routes = [
  ...
  },
  {
    name: "index",
    path: "/",
    component: () => import('./index-CFJF4Cug.mjs')
  }
];
...
const routerOptions1 = {
  routes: (_routes2) => {
    ...
    if (ssrContext == null ? void 0 : ssrContext.event.context.subdomain) {
      subdomain = ssrContext == null ? void 0 : ssrContext.event.context.subdomain;
      useCookie("subdomain").value = subdomain;
    }
    if (subdomain) {
      const userRoute = _routes2.filter((i) => {
        if (page) return i.path == `/${subdomain}/${page}`;
        else return i.path == `/${subdomain}`;
      });
      const userRouteMapped = userRoute.map((i) => ({
        ...i,
        path: page ? i.path === `/${subdomain}/${page}` ? i.path.replace(`/${subdomain}/${page}`, `/${page}`) : i.path.replace(`/${subdomain}/${page}/`, `/${page}`) : i.path === `/${subdomain}` ? i.path.replace(`/${subdomain}`, "/") : i.path.replace(`/${subdomain}/`, "/")
      }));
  ...
  scrollBehavior(to, from, savedPosition) {
    ...
    if (to.hash) {
      ...
      return { left: 0, top: ((el == null ? void 0 : el.offsetTop) ?? 0) - 30, behavior: "smooth" };
    }
...
const _sfc_main$1 = {
  ...
  setup(__props) {
    ...
    const stack = void 0;
    const _Error404 = defineAsyncComponent(() => import('./error-404-BneDcP-e.mjs'));
    const _Error = defineAsyncComponent(() => import('./error-500-CXF6UKZK.mjs'));
